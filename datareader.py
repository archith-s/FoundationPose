...
class CustomRosReader(BopBaseReader):
    def __init__(self, base_dir, zfar=np.inf):
        # We manually bypass the BopBaseReader's strict __init__ 
        ...
        for k in info:
            self.K_table[f'{int(k):06d}'] = np.array(info[k]['cam_K']).reshape(3,3)
            self.bop_depth_scale = info[k].get('depth_scale', 1.0)
        
        # Set a default K if the table is smaller than the image count
        first_key = list(self.K_table.keys())[0]
        self.K = self.K_table[first_key]

        # 3. Handle Ground Truth (Make it optional)
        if os.path.exists(f'{self.base_dir}/scene_gt.json'):
            with open(f'{self.base_dir}/scene_gt.json', 'r') as ff:
                self.scene_gt = json.load(ff)
        else:
            self.scene_gt = None
            logging.info("scene_gt.json not found or being ignored. Tracking will use manual/initial mask.")

    def get_depth(self, i):
        id_str = self.id_strs[i]
        # Direct path construction
        depth_file = os.path.join(self.base_dir, 'depth', f'{id_str}.png')
        
        # Debug print: This will show up in your terminal
        # print(f"DEBUG: Attempting to load: {depth_file}")

        # Use cv2.IMREAD_UNCHANGED to ensure we get the 16-bit values
        depth = cv2.imread(depth_file, cv2.IMREAD_UNCHANGED)
        
        if depth is None:
            # If the direct path failed, check if the file actually exists on disk
            if not os.path.exists(depth_file):
                raise FileNotFoundError(f"File does not exist on disk: {depth_file}")
            else:
                raise IOError(f"File exists but cv2 could not read it: {depth_file}. Is it a valid PNG?")

        # Convert to meters
        depth = depth.astype(np.float32) * 1e-3 * self.bop_depth_scale
    ...
    def get_mask(self, i):
        id_str = self.id_strs[i]
        mask_file = os.path.join(self.base_dir, 'segmented', f'{id_str}.png')
        
        # Load as grayscale (the 0 flag forces it to 2D)
        mask = cv2.imread(mask_file, 0)
        
        if mask is None:
            # Fallback for naming
            mask_file = os.path.join(self.base_dir, 'segmented', f'{int(id_str)}.png')
            mask = cv2.imread(mask_file, 0)
            
        if mask is None:
            raise FileNotFoundError(f"Mask not found at {mask_file}")

        # Ensure it's a binary boolean mask (True where the object is)
        # This converts (480, 640) to a boolean array that matches the depth shape
        return mask > 0
